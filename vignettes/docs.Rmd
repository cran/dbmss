...
This vignette contains a quick introduction to the package (first section) and its reference guide (second section).
...
Exploratory statistics of point patterns widely rely on Ripley's seminal work [@Ripley1976; @Ripley1977], namely the $K$ function. 
...
We propose extended possibilities to evaluate confidence envelopes, including _global envelopes_ [@Duranton2005], a goodness-of-fit test [@Diggle1983] and an analytical test [@Lang2010; @Marcon2013].
...
- Ripley's $g$ and $K$, and $K$'s normalization, Besag's $L$ [-@Besag1977]; 
...
X <- wmppp(data.frame(X=Emergencies$X, Y=Emergencies$Y, PointType=Category), win=Region)
...
It represents the distribution of trees in a 4.1-ha tropical forest plot in the Paracou field station in French Guiana [@Gourlet2004]. 
...
A Goodness-of-fit test for $K$ has been proposed by @Diggle1983, applied to $K$ by @Loosmore2006 and to $M$ by @Marcon2012. 
...
The `Ktest` has been developed by Lang and Marcon [@Lang2010; @Marcon2013]. 
